...
%--------------------------------------------------------------------------
    ...
    Type      = arrayfun(@(i)find(Problem.encoding==i),1:9,'UniformOutput',false);
    ...
    if ~isempty(Type{8})        % Permutation variables parar aqui
        Offspring(:,Type{8}) = GApermutationMCMACompara(Parent1(:,Type{8}),Parent2(:,Type{8}),Problem);
    end
    if ~isempty(Type{9})        % Permutation variables parar aqui
        if ~isempty (find(Parent > (cell2mat(Problem.parameter(2))*cell2mat(Problem.parameter(3)))))
            find(Parent > (cell2mat(Problem.parameter(2))*cell2mat(Problem.parameter(3))))
        end
        Offspring(:,Type{9}) = GApermutationMCMAGA(Parent1(:,Type{9}),Parent2(:,Type{9}),Problem);
    end
...
function Offspring = GApermutationMCMACompara(Parent1,Parent2,proC)
% Genetic operators for permutation variables
% Added by Manoel Aranda de Almeida 25/05/2023
% to include GA operator version for Many-Core
    ...
    Offspring = MCMAComparapermutationGA(Parent1,Parent2,proC);
end

function Offspring = GApermutationMCMAGA(Parent1,Parent2,proC)
% Genetic operators for permutation variables
    ...
    s = randi(D,1,2*N);
    for i = 1 : 2*N
%         temTroca = Offspring(i,k);
%         Offspring(i,k) = Offspring(i,s);
%         Offspring(i,s) = temTroca;
        if s(i) < k(i)
            Offspring(i,:) = Offspring(i,[1:s(i)-1,k(i),s(i):k(i)-1,k(i)+1:end]);
        elseif s(i) > k(i)
            Offspring(i,:) = Offspring(i,[1:k(i)-1,k(i)+1:s(i)-1,k(i),s(i):end]);
...
%         if ~isempty (find(Offspring > (cell2mat(proC.parameter(2))*cell2mat(proC.parameter(3)))))
%             (find(Offspring > (cell2mat(proC.parameter(2))*cell2mat(proC.parameter(3)))))
%         end
end